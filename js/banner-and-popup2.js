...
var bannerShown = true,
    popupShown = true,
    titlesBanner = true,
    titlesBannerLocation = "QuizzesandActivities";
...
$(document).ready(function () {
    ...
    if (titlesBanner && !titlesBannerAdded) {
        makeTitlesBanner();
    }
...
function setBodyClassForItem() {
    var bodyClassForItem;
    var iframeParentClass = window.frameElement.classList[0];
    console.log(iframeParentClass);
    if (iframeParentClass === 'banner-iframe') {
        bodyClassForItem = "bannerItem";
    }

    if (iframeParentClass === 'popup-iframe') {
        bodyClassForItem = "popupItem";
    }

    console.log(bodyClassForItem);
    $('body').addClass(bodyClassForItem);
...
function makeTitlesBanner() {
    // console.log('^^^^^^^^^^^^^makeTitlesBanner');
    var $insertAfter;
    $.get("banner-for-titles.html", function (data) {

        var checkIfElemIsThereYet = setInterval(function () {
            $insertAfter = $parentBody.find('.PANEL.level1#QuizzesandActivities h2');
            // console.log($insertAfter);
            if ($insertAfter.length > 0) {
                $insertAfter.after(data);
                titlesBannerAdded = true;
                clearInterval(checkIfElemIsThereYet);
            }
        }, 250);
    });
}